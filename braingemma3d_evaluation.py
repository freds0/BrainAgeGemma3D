#!/usr/bin/env python3
"""
BrainGemma3D Scientific Evaluation Script (Publication Grade)
===========================================================
Performs a complete evaluation on the "Unseen" Test Set to produce
...
Usage:
    python braingemma3d_evaluation.py \
        --checkpoint-dir checkpoints/EXPERIMENT/phase2b_final \
...
# --- IMPORTS FROM THE NEW SEPARATE FILES ---
try:
...
try:
    ...
    print("⚠️  WARNING: nltk or rouge-score not installed.")
    NLG_AVAILABLE = False
...
# ============================================================================
# 1. CLINICAL METRICS CLASS (The core of medical validation)
# ============================================================================
class ClinicalMetrics:
    ...
    def extract_entities(self, text: str) -> Dict[str, Set[str]]:
        ...
        for cat, keywords in self.categories.items():
            for kw in keywords:
                # Match whole word (avoid "low" matching inside "slow")
                if f" {kw} " in f" {text} ": 
                    found[cat].add(kw)
        return found
    ...
    def compute_scores(self, reference: str, hypothesis: str) -> Dict[str, float]:
        ...
        for cat in self.categories:
            ...
            # If GT has no entities, F1 is 1.0 if Hyp also has none, otherwise 0.0
            if len(ref_set) == 0:
...
def main():
    ...
    # Data - MUST BE IDENTICAL TO TRAINING FOR THE CORRECT TEST SET
    ...
    # Generation Params (Conservative for benchmarking)
    ...
    # 1. DATASET RECONSTRUCTION & SPLIT (The fundamental part)
    ...
    # Reconstruct the entire dataset with the "balanced" logic
    ...
    # Perform the 70/10/20 split EXACTLY as in training
    _, _, test_data = make_group_split(dataset, seed=args.seed, train_frac=0.7, val_frac=0.1)
    
    print(f"✅ Test Set Isolated: {len(test_data)} patients")
    ...
    for i, ex in enumerate(tqdm(test_data)):
        try:
            # Load volume (import robust function from training)
    ...
    # 4. COMPUTE BERTSCORE (Global)
    # Free MedGemma model memory to make room for BERT
    del model
    torch.cuda.empty_cache()
    ...
    final_stats = {"config": vars(args), "metrics": {}}
    ...
    # 6. PRINT FINAL TABLE (Copy-Paste for the Paper)
    print("\n" + "="*70)
